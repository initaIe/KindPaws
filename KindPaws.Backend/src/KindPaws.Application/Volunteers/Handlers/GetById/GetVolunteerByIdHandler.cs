...
public class GetVolunteerByIdHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (volunteerResult.Value.Description != null)
        {
            description = volunteerResult.Value.Description.Value;
        }
        ...
        if (volunteerResult.Value.SocialNetworkList.SocialNetworks is { Count: > 0 })
        {
            socialNetworks = volunteerResult.Value.SocialNetworkList.SocialNetworks
                .Select(x => new SocialNetworkDTO(x.Name, x.Link));
        }
        
        IEnumerable<RequisiteDTO> requisites = [];
        if (volunteerResult.Value.RequisiteList.Requisites is { Count: > 0 })
        {
            requisites = volunteerResult.Value.RequisiteList.Requisites
                .Select(x => new RequisiteDTO(x.Name, x.Description));
        }
