...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    public Result<Error> AddPermission(PermissionId permissionId)
    {
        ...
        if (isPermissionAlreadyExist)
            return ErrorsGeneral.RecordAlreadyExist(nameof(Permission), nameof(PermissionId));
    ...
    public Result<Error> DeletePermission(PermissionId permissionId)
    {
        ...
        if (!isPermissionExist)
            return ErrorsGeneral.RecordNotFound(nameof(Permission), nameof(PermissionId));
